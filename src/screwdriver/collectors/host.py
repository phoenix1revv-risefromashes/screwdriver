...
def collect_identity() -> HostIdentity:
    ...
    if primary_group is not None:
        groups = sorted({*groups, primary_group.gr_name})
...
def collect_platform(cpu_vendor: str | None = None) -> PlatformInfo:
    ...
    detection_text = " ".join(
        value for value in (manufacturer, product_name, board_name, compatible) if value
    ).lower()
...
def collect_cpu() -> CPUInfo:
    ...
    return CPUInfo(
        ...
        online_cpus=_count_cpu_range(
            _read_text(Path("/sys/devices/system/cpu/online")), logical_cpus
        ),
...
def collect_storage_devices() -> list[StorageDevice]:
    ...
    for item in cast(list[object], raw["blockdevices"]):
        ...
        devices.append(
            StorageDevice(
                ...
                media_type=_storage_media_type(name, connection, rotational),
...
def collect_gpus(platform_info: PlatformInfo) -> list[GPUInfo]:
    ...
    nvidia_output = _run_command(
        [
            "nvidia-smi",
            "--query-gpu=name,driver_version,pci.bus_id,memory.total,utilization.gpu,temperature.gpu",
    ...
    pci_output = _run_command(["lspci", "-D", "-nnk"])
...
def collect_power(platform_info: PlatformInfo) -> PowerInfo:
    ...
    if seconds_remaining in {psutil.POWER_TIME_UNKNOWN, psutil.POWER_TIME_UNLIMITED}:
...
def _create_findings(snapshot: SystemSnapshot) -> list[Finding]:
...
def _collect_partition_records(record: dict[str, object]) -> list[StoragePartition]:
...
def _read_cpuinfo() -> dict[str, str]:
    text = _read_text(Path("/proc/cpuinfo"), strip_nulls=False) or ""
    data: dict[str, str] = {}
...
def _read_lscpu() -> dict[str, str]:
    output = _run_command(["lscpu", "--json"])
    if output is None:
        return {}
    try:
        raw = json.loads(output)
        ...
        return {}
    if not isinstance(raw, dict) or not isinstance(raw.get("lscpu"), list):
        return {}
    data: dict[str, str] = {}
    for item in cast(list[object], raw["lscpu"]):
        if not isinstance(item, dict):
            continue
...
def _collect_jetson_details() -> dict[str, str | int | float | bool | None]:
    details: dict[str, str | int | float | bool | None] = {}
    release = _read_text(Path("/etc/nv_tegra_release"))
    if release:
        match = re.search(r"R(\d+)\s*\(release\).*REVISION:\s*([\d.]+)", release)
        details["l4t"] = f"{match.group(1)}.{match.group(2)}" if match else release.splitlines()[0]
    jetpack = _run_command(["dpkg-query", "-W", "-f=${Version}", "nvidia-jetpack"])
    if jetpack:
...
def _collect_raspberry_pi_details() -> dict[str, str | int | float | bool | None]:
    details: dict[str, str | int | float | bool | None] = {}
    revision = _read_cpuinfo().get("revision")
    if revision:
        details["board_revision"] = revision
    throttled = _run_command(["vcgencmd", "get_throttled"])
    if throttled:
        details["throttled_state"] = throttled.split("=", 1)[-1]
    firmware = _run_command(["vcgencmd", "version"])
    if firmware:
...
def _parse_lspci_gpus(output: str) -> list[GPUInfo]:
    gpus: list[GPUInfo] = []
    current: GPUInfo | None = None
    for line in output.splitlines():
        if line and not line[0].isspace():
            current = None
            match = re.match(r"(\S+)\s+(.+?(?:VGA|3D|Display).+?):\s+(.+)", line)
            if match:
...
def _collect_default_route() -> tuple[str | None, str | None]:
    output = _run_command(["ip", "-j", "route", "show", "default"])
    if output is None:
        return None, None
    try:
        raw = json.loads(output)
    ...
    for item in cast(list[object], raw):
        if isinstance(item, dict):
            return _as_string(item.get("dev")), _as_string(item.get("gateway"))
    return None, None


def _collect_dns_servers() -> list[str]:
    text = _read_text(Path("/etc/resolv.conf"), strip_nulls=False) or ""
    servers: list[str] = []
    for line in text.splitlines():
        fields = line.split()
...
def _network_kind(record: dict[str, object]) -> str:
    ...
    if re.fullmatch(r"usb\d+", name) or name.startswith("l4tbr"):
        return "USB-gadget network"
    if info_kind:
        return info_kind
    if name.startswith(("docker", "br-", "veth", "virbr", "tun", "tap")):
        return "virtual"
    if Path(f"/sys/class/net/{name}/wireless").exists() or name.startswith(("wl", "wlan")):
        return "wifi"
    if link_type == "ether":
        return "ethernet"
...
def _storage_media_type(name: str, connection: str | None, rotational: bool) -> str:
...
def _run_command(arguments: list[str], timeout: float = 3.0) -> str | None:
    try:
...
def _read_text(path: Path, *, strip_nulls: bool = True) -> str | None:
    try:
...
def _group_for_gid(gid: int) -> grp.struct_group | None:
    try:
...
def _count_cpu_range(value: str | None, fallback: int) -> int:
    if not value:
        return fallback
    count = 0
    try:
